                            ...
                            <div class="grid-items">
                                <ul>
                                 @foreach ($talks as $talk)
                                    <li>
                                    <a href="/mentor/{{$talk->id}}" style="color:#000;text-decoration:none"><img src="{{{secure_asset($talk->pic0_path)}}}" alt="">
                                    <h4>{{ $talk->title }}</h4>
                                    <h4>いいね:{{ $talk->likes_count }}人 トーク:{{ $talk->applications_count }}人</h4>
                                    <h4>{{ $talk->category->category_name }}</h4>
                                    <h4></a>
                                    <a href="/mentor/{{$talk->id}}"><button type="button" class="btn2 btn-default btn-sm btn-block">詳細を見る</button></a>
                                    </li>
                                @endforeach
                                </ul>
                            </div>
